...
export const ResetPasswordForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await serviceAuth.resetPasswordStart({email: email.trim()});
      if (response.success) {
        setIsSuccess(true);
      }
    } catch {
      setSubmitError('Не удалось отправить письмо для восстановления пароля...');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View className="w-full gap-4">
      ...
      <AuthSubmitButton
        ...
        {isSuccess ? 'Закрыть' : 'Восстановить пароль'}
      </AuthSubmitButton>
    </View>
  );
};
